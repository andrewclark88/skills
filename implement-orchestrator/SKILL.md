...
3. **Engineering principles skill** — invoke `/engineering-principles` to load architectural and code-level principles (Ports & Adapters, Single Source of Truth, Generated Contracts, Fail Fast)
4. **Pattern files** — if the project has documented patterns (e.g., `.claude/rules/`, `.claude/skills/patterns/`), read them
5. **Referenced spec/architecture/roadmap docs** — if the design references other docs, read the relevant sections
...
#### 1a. Load domain briefs from the knowledge index
Check `docs/knowledge-index.yaml` for briefs whose `blocks_phase` matches the current phase, or whose topic matches the implementation target. Read these briefs before dispatching any subagent. They contain validated research that prevents bugs and shortcuts. Include relevant brief findings in subagent prompts.

#### 1b. Read the design document thoroughly
Read every unit — understand the full scope, dependencies between units, and the implementation order.

#### 1c. Read project documentation and principles
- **CLAUDE.md** (project root and `.claude/` if they exist) — conventions, commands, patterns, project structure
- **Engineering principles** — invoke `/engineering-principles` to load architectural and code-level principles
...
#### 1d. Ground in the actual codebase
...
5. **Principles** — instruct the agent: "Before writing any code, invoke `/engineering-principles` to load the project's architectural and code-level principles. Follow them throughout implementation."
