...
**Q-Ev3 conditional-severity extension (added 2026-05-12):** `/research-program` Phase 8 Evaluator now supports conditional-severity master traps. Reads both scalar `severity: <enum>` and conditional `severity_conditional: {condition, true, false}` blocks; preserves input structure on reassessment (scalar→scalar, conditional→conditional); may propose scalar→conditional promotions as CLAIM-tagged `severity_model_proposed` (not auto-applied; adjudication is downstream). Per ds-engine architecture-phase decision (Phase 7 Run #8 Q-Ev3 + Q9-8 + BLOCKING #4). See `research-program/SKILL.md` Phase 8.
