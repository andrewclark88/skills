...
5. Use the **engineering-principles** skill — apply Ports & Adapters, Single Source of Truth, Generated Contracts, and Fail Fast to your design decisions
...
Run `/knowledge-index` (or read `docs/knowledge-index.yaml` directly). Identify every architecture doc, brief, and research primer relevant to the design target. **Read these before starting design.** Designs based on missed context cause rewrites — checking the index is non-negotiable.

Specifically look for:
- The project's north-star doc (vision, principles)
- The architecture doc (modules, conventions)
- Any briefs whose `blocks_phase` matches the current phase
- Any briefs tagged with relevant domains for the design target

Loading these is part of the design step, not preamble — they shape the output.
