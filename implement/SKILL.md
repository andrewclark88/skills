...
5. Use the **engineering-principles** skill — apply Ports & Adapters, Single Source of Truth, Generated Contracts, and Fail Fast when writing code
...
2. **Load domain briefs from the knowledge index.** Check `docs/knowledge-index.yaml` for briefs whose `blocks_phase` matches the current phase, or whose topic matches the implementation target. Read these briefs before writing any code. They contain validated research that prevents bugs and shortcuts.
3. Use the **patterns** skill to read relevant patterns for the code you're about to write
4. **Read research documents**: If the design references external libraries or APIs, find the project's research docs for those topics — validated API usage patterns, version-specific guidance, and known gotchas.
